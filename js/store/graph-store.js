...
class GraphStore {
	...
	constructor() {
		...
		subscribe(EVENTS.GRAPH_EDGE_CREATE_REQUESTED, ({ payload }) => {
			...
			this.addEdge(payload.edge ?? payload, {
				selectAfterCreate: Boolean(payload?.selectAfterCreate),
			});
		});
	...
	load(documentLike, { fromHistory = false, reason = "load" } = {}) {
		...
		if (!validation.valid) {
			throw new Error(
				`Invalid graph document: ${validation.errors.join(", ")}`,
			);
		}
		...
		publish(EVENTS.GRAPH_VIEWPORT_CHANGED, {
			...(snapshot?.viewport ?? { x: 0, y: 0, z: 0, zoom: 1 }),
			origin: "graph-store",
		});
	...
	save() {
		const snapshot = this.getDocument();
		publish(EVENTS.GRAPH_DOCUMENT_SAVED, {
			document: snapshot,
			origin: "graph-store",
		});
		return snapshot;
	...
	getNodesByType(type) {
		...
		return clone(findNodesByType(this.#document, type));
	}

	getChunksByTier(tier) {
		if (!this.#document) return [];
		return clone(graphDocument.getChunksByTier(this.#document, tier));
	}

	getConnectedNodes(nodeId) {
		if (!this.#document) return [];
		return clone(getConnectedNodes(this.#document, nodeId));
	}

	getClusters() {
		return this.#document ? clone(this.#document.clusters ?? []) : [];
	}

	getClusterForNode(nodeId) {
		const clusters = this.#document?.clusters ?? [];
		return (
			clone(clusters.find((c) => (c.member_ids ?? []).includes(nodeId))) ?? null
		);
	}

	/* ── Cluster mutations ── */

	addCluster(clusterDef) {
		if (!this.#document) return null;
		this.#applyMutation(() => {
		...
		publish(EVENTS.CAR_CLUSTER_FORMED, {
			cluster: clone(clusterDef),
			origin: "graph-store",
		});
		this.#emitDocumentChanged("cluster_formed");
		return clone(clusterDef);
	}

	updateCluster(clusterId, patch) {
		if (!this.#document) return null;
		this.#applyMutation(() => {
			this.#document.clusters = (this.#document.clusters ?? []).map((c) =>
				c.id === clusterId ? { ...c, ...patch } : c,
			);
		});
		this.#emitDocumentChanged("cluster_updated", { clusterId });
	...
	removeCluster(clusterId) {
		...
		this.#applyMutation(() => {
			this.#document.clusters = (this.#document.clusters ?? []).filter(
				(c) => c.id !== clusterId,
			);
			// Also remove clusters_with edges
			this.#document.edges = (this.#document.edges ?? []).filter(
				(edge) =>
					!(
						edge.type === "clusters_with" &&
						(edge.source === clusterId || edge.target === clusterId)
					),
			);
		});
		publish(EVENTS.CAR_CLUSTER_DISSOLVED, { clusterId, origin: "graph-store" });
		this.#emitDocumentChanged("cluster_dissolved", { clusterId });
		return cluster;
	}
	...
	setSelection(nodeIds = []) {
		...
		this.clearSelectedEdge({ emitChange: false });
		const validIds = unique(nodeIds).filter((nodeId) =>
			Boolean(findNodeById(this.#document, nodeId)),
		);
	...
	updateViewport(nextViewport = {}) {
		...
		const prev = this.#document.viewport ?? { x: 0, y: 0, z: 0, zoom: 1 };
		...
		const nextZ = Number(nextViewport.z ?? prev.z ?? 0);

		this.#document.viewport = {
			x: Number.isFinite(nextX) ? nextX : (prev.x ?? 0),
			y: Number.isFinite(nextY) ? nextY : (prev.y ?? 0),
			z: Number.isFinite(nextZ) ? nextZ : (prev.z ?? 0),
			zoom: Number.isFinite(nextZoom) ? nextZoom : (prev.zoom ?? 1),
			rotationX: Number(nextViewport.rotationX ?? prev.rotationX ?? 0),
			rotationY: Number(nextViewport.rotationY ?? prev.rotationY ?? 0),
		};
	...
	updateNode(nodeId, patch) {
		...
		if (normalizedPatch.position) {
			normalizedPatch.position = {
				...
				z: Number(normalizedPatch.position.z ?? existing.position?.z ?? 0),
			};
		}
	...
	removeNode(nodeId) {
		...
		this.#applyMutation(() => {
			...
			this.#document.clusters = (this.#document.clusters ?? []).map((c) => ({
				...c,
				member_ids: (c.member_ids ?? []).filter((id) => id !== nodeId),
			}));
		});

		if (
			this.#selectedEdgeId &&
			!findEdgeById(this.#document, this.#selectedEdgeId)
		) {
			this.clearSelectedEdge({ emitChange: false });
		}
	...
	updateEdge(edgeId, patch) {
		...
		if (normalizedPatch.type != null)
			normalizedPatch.type = String(normalizedPatch.type);
		if (normalizedPatch.label != null)
			normalizedPatch.label = String(normalizedPatch.label);
	...
	recordAccess(nodeId) {
		if (!this.#document) return;
		const node = findNodeById(this.#document, nodeId);
		if (!node || node.type !== "chunk") return;
	...
	#syncSelectionFromDocument() {
		...
		if (Array.isArray(selected) && selected.length) {
			this.#selectedNodeIds = unique(selected).filter((nodeId) =>
				Boolean(findNodeById(this.#document, nodeId)),
			);
			this.#selectedEdgeId = null;
			return;
		...
		this.#selectedEdgeId =
			selectedEdgeId && findEdgeById(this.#document, selectedEdgeId)
				? selectedEdgeId
				: null;
	}
